...
function Invoke-AES-CBC-Encrypt {
    param(
        [string]$PlainText,
        [string]$Key, 
        [string]$IV
    ...
    $bytesWritten = 0
    $status = [PowH.Core.Cryptography.BCrypt]::BCryptGetProperty($hAlgorithm, "ObjectLength", $keyObjLength, 4, [ref]$bytesWritten, 0)
    if ($status -ne $STATUS_SUCCESS) {
    ...
    # Set CBC mode
    $status = [PowH.Core.Cryptography.BCrypt]::BCryptSetProperty($hAlgorithm, $BCRYPT_CHAINING_MODE, [System.Text.Encoding]::Unicode.GetBytes($BCRYPT_CHAIN_MODE_CBC), 16, 0)
    if ($status -ne $STATUS_SUCCESS) {
    ...
    $status = [PowH.Core.Cryptography.BCrypt]::BCryptGenerateSymmetricKey($hAlgorithm, [ref]$hKey, $pbKeyObject, $cbKeyObject, [System.Text.Encoding]::Unicode.GetBytes($Key), 32, 0)
    if ($status -ne $STATUS_SUCCESS) {
    ...
    $status = [PowH.Core.Cryptography.BCrypt]::BCryptEncrypt($hKey, $plaintextBytes, $plaintextBytes.Length, [IntPtr]::Zero, 
...
function Invoke-AES-CBC-Decrypt {
    ...
    $outputBuffer = [byte[]]::new($paddedLength)
    $bytesWritten = 0
    $status = [PowH.Core.Cryptography.BCrypt]::BCryptDecrypt($hKey, $ciphertext, $ciphertext.Length, [IntPtr]::Zero, 
    [System.Text.Encoding]::Unicode.GetBytes($IV), 16, $outputBuffer, $outputBuffer.Length, 
    [ref]$bytesWritten, $BCRYPT_BLOCK_PADDING)
    if ($status -ne $STATUS_SUCCESS) {
